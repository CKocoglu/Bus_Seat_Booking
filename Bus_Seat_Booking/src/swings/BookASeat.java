...
public class BookASeat {
	...
	public BookASeat(JFrame superF) 
	{
		...
		txtCityCode.setBounds(134, 102, 32, 20);
		frmBookASeat.getContentPane().add(txtCityCode);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) 
			{
				...
				Passenger p = new Passenger(txtName.getText(), 
						txtSurname.getText(), 
						(rbtnGenderFemale.isSelected() ? "female":"male"), ph);
				
				boolean retMessage = MenuPage.bus.bookSeat(p, Integer.parseInt(cbxSeatNumber.getSelectedItem().toString()));
					
				if(retMessage)
				{
					JOptionPane.showMessageDialog(frmBookASeat, "Successfully added!");
				}
				else
				{
					JOptionPane.showMessageDialog(frmBookASeat, "Error! This seat is full");
